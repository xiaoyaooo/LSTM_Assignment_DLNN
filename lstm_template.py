...
def backward(activations, clipping=True):
    ...
    for t in reversed(range(input_length)):
        ...
        dhs = np.dot(Why.T, do) + dhnext
        dcs = dhs * os[t] * dtanh(np.tanh(cs[t])) + dcnext #??dc = dcnext

        dos_pre = dhs * np.tanh(cs[t]) * dsigmoid(os[t])
        dWo += np.dot(dos_pre, zs[t].T)
        dbo += np.sum(dos_pre, axis=-1, keepdims=True)


        dc_s_pre = dcs * ins[t] * dtanh(c_s[t])
        dWc += np.dot(dc_s_pre, zs[t].T)
        dbc += np.sum(dc_s_pre, axis=-1, keepdims=True)

        dins_pre = dcs * c_s[t] * dsigmoid(ins[t])
        dWi += np.dot(dins_pre, zs[t].T)
        dbi += np.sum(dins_pre, axis=-1, keepdims=True)

        dfs_pre = dcs * cs[t - 1] * dsigmoid(fs[t])
        dWf += np.dot(dfs_pre, zs[t].T)
        dbf += np.sum(dfs_pre, axis=-1, keepdims=True)

        dzs = np.dot(Wo.T, dos_pre) + np.dot(Wc.T, dc_s_pre) \
                + np.dot(Wi.T, dins_pre) + np.dot(Wf.T, dfs_pre)
